...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings: torch.Tensor) -> None:
        ...
        """
        # *** BEGIN YOUR CODE *** #
        c = self.config  # Configurations
        self.word_embed = nn.Embedding.from_pretrained(word_embeddings, freeze=False)

        # First arg is the number of embeddings, the second is the embedding size
        self.tag_embed = nn.Embedding(c.n_tag_ids, c.embed_size)
        self.deprel_embed = nn.Embedding(c.n_deprel_ids, c.embed_size)
        # *** END YOUR CODE *** #
    ...
    def create_net_layers(self) -> None:
        ...
        input_size = N * c.embed_size
        self.hidden_layer = nn.Linear(input_size, c.hidden_size)
    ...
    def reshape_embedded(self, input_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # *** BEGIN YOUR CODE *** #
        reshaped_batch = torch.reshape(input_batch, (input_batch.shape[0], -1))
        # *** END YOUR CODE *** #
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # *** BEGIN YOUR CODE *** #
        # Embedding receives input from tensor of arbitrary shape containing the indices to extract
        # Given input of size *, the output has size (*, embedding_dim)
        w = self.word_embed(word_id_batch)     # (B, n_word_features, embed_size)
        t = self.tag_embed(tag_id_batch)       # (B, n_tag_features, embed_size)
        d = self.deprel_embed(deprel_id_batch) # (B, n_deprel_features, embed_size)

        # Reshape the embedded batch tensor
        w_reshaped = self.reshape_embedded(w)           # (B, n_word_features * embed_size)
        t_reshaped = self.reshape_embedded(t)           # (B, n_tag_features * embed_size)
        d_reshaped = self.reshape_embedded(d)           # (B, n_deprel_features * embed_size)

        # Concatenate the reshaped embedded inputs together
        # Concat along dim=1 (Horizontal Concatenation)
        x = torch.cat((w_reshaped, t_reshaped, d_reshaped), dim=1)  # (B, N * embed_size)
    ...
    def forward(self,
        ...
        # *** BEGIN YOUR CODE *** #
        # x has size (B, N * embed_size)
        # hidden layer has size (N * embed_size, hidden_size)
        c = self.config
        h = self.hidden_layer(x)       # Apply linear transformation using the hidden layer    (B, hidden_size)
        activated_h = torch.relu(h)    # Apply ReLU activation function                        (B, hidden_size)
        h_drop = torch.dropout(activated_h, c.dropout, self.training)                        # (B, hidden_size)

        # output layer has size (hidden_size, n_classes)
        pred = self.output_layer(h_drop)                                                     # (B, n_classes)
        # *** END YOUR CODE *** #
        return pred
