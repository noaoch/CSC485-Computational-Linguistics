...
class GraphDepModel(nn.Module):
    ...
    def create_arc_layers(self) -> None:
        ...
        * Initialize self.arc_W and self.arc_B according to a uniform
        ...
        # S is a matrix of size n+1 x p, where we have n+1 input vectors, and each vector has dimension p = self.pt_width
        # MLP transforms this into a vector of dimension cfg.ARC_DIM
        # So D_A is a matrix of size (n+1) x cfg.ARC_DIM

        self.arc_W = nn.Parameter(torch.empty(cfg.ARC_DIM, cfg.ARC_DIM))
        self.arc_B = nn.Parameter(torch.empty(cfg.ARC_DIM))
    ...
    def score_arcs(self, arc_head: Tensor, arc_dep: Tensor) -> Tensor:
        ...
        arc_scores1 = torch.einsum('byd,dd,bxd->byx', arc_dep, self.arc_W, arc_head)
        arc_scores2 = torch.einsum('bxd,d->bx', arc_head, self.arc_B)
        arc_scores = arc_scores1 + arc_scores2[:,:,None]
    ...
    def create_label_layers(self) -> None:
        ...
        * Initialize self.label_B to zeros.
        ...
        # l_ij has dimension n_deprels
        # H has dim (n+1) x cfg.LABEL_DIM

        self.label_W = nn.Parameter(torch.empty(cfg.LABEL_DIM, cfg.LABEL_DIM, self._n_deprels))
        self.label_h_W = nn.Parameter(torch.empty(cfg.LABEL_DIM, self._n_deprels))
        self.label_d_W = nn.Parameter(torch.empty(cfg.LABEL_DIM, self._n_deprels))
        self.label_B = nn.Parameter(torch.empty(self._n_deprels))
    ...
    def score_labels(self, label_head: Tensor, label_dep: Tensor) -> Tensor:
        ...
        # *** BEGIN YOUR CODE *** #
        # Compute using the equation
        DLWL = torch.einsum('byd,ddn->bydn', label_dep, self.label_W)
        HT = torch.einsum('bxd->dxb', label_head)
        label_scores1 = torch.einsum('bydn,dxb->byxn', DLWL, HT)

        label_scores2 = torch.einsum('bxd,dn->bxn', label_head, self.label_h_W)

        label_scores3 = torch.einsum('byd,dn->byn', label_dep, self.label_d_W)

        label_scores4 = self.label_B.unsqueeze(0).unsqueeze(0).unsqueeze(0)

        label_scores = label_scores1 + label_scores2.unsqueeze(1) + label_scores3.unsqueeze(2) + label_scores4
        # *** END YOUR CODE *** #
        return label_scores
    ...
    def mask_possible(self, shape: T.Sequence[int]):
        ...
        for b in range(shape[0]):
            # For each dependent in the sentence
            for i in range(shape[1]):
                # for every label, eliminate cycle
                for k in range(shape[3]):
                    mask[b][i][i][k] = False

                # if head of dependent is not root, then the label cannot be root
                for j in range(1, shape[2]):
                    mask[b][i][j][0] = False

                # If the label is not root, then its impossible for the head to be root
                for k in range(1, shape[3]):
                    mask[b][i][0][k] = False

            
            # For each head in the sentence
            for j in range(shape[2]):
                # eliminate cycle
                for k in range(shape[3]):
                    mask[b][j][j][k] = False
                    mask[b][0][j][k] = False  # # The root cannot be the dependent of any other word
        # *** END YOUR CODE *** #
        return mask
